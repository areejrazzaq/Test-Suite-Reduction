...
var CSVProcessor = (function(path){
	...
	var tolerate = function(suite){
		let curScore = CSVProcessor.getMutationScore(suite);
		let tolerance = mutationScore-alpha
	...
	return{
		performSelection: function(){
			...
			let debugStr = null
			let iter = 0
			while (linesArr.length > 0 && !tolerate(seletedSet)){
		...
		outputCSVHeader: function(){
			if (!fs.existsSync("greedyMinOutputs.csv")){
				var header = "DateTime,Name,alpha,TotalMutants,ExecutionTime(ms),ExecutionTime,MutationScoreOriginal,MutationScoreReduced,ReducedSet,OriginalSetSize,ReducedSetSize"
...
const mutationScore = CSVProcessor.getMutationScore(arr);
...
var str = datetime+","+outputName+","+ alpha+","+CSVProcessor.getTotalMutants()+","+time+"ms"+","+minutes+"m "+ (seconds-minutes*60)+"s " + (time - seconds*1000)+ "ms"+","+CSVProcessor.getMutationScore(arr)+","+CSVProcessor.getMutationScore(reducedSet)+","+reducedSet.join(" ")+","+CSVProcessor.getHeader().length+","+reducedSet.length+"\n";
