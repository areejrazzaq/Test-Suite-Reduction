...
function createOutput() {
    if (!fs.existsSync("linMin-ensemble.csv")) {
        ...
        fs.appendFileSync("linMin-ensemble.csv", row1 + "\n", 'utf8');
        fs.appendFileSync("linMin-ensemble.csv", row2 + "\n", 'utf8');
    }
}

function outputToCSV(str) {
    fs.appendFileSync("linMin-ensemble.csv", str, 'utf8');
}
...
function runScript(pathToFile, callback) {
    const output = execFileSync("node", ["linMin.js" ,pathToFile,alpha])
