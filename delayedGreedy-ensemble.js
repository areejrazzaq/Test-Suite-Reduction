...
function outputToCSV(str) {
    fs.appendFileSync("delayedGreedy-ensemble.csv", str, 'utf8');
}
...
function runScript(pathToFile, callback) {
    const output = execFileSync("node", ["delayedGreedy.js" ,pathToFile,alpha])
...
while (count < runs ) {
    ...
    let bol = runScript(pathToFile, function(err) {
		console.log(`${chalk.green("thrown error")}`)
        if (err) throw err;
    });
    if(bol==0){
        count = temp;
